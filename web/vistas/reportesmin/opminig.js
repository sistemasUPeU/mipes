$(document).ready(function () {
    $("#titulo").append($("#nIglesiaUser").val());
    activeCalendar();
});
...
function graficar(feci, fecf) {
    var id = $("#idIglesiaUser").val();
    var url = "rep?opc=reportmin&op=2";
    var data = "";
    data += "idiglesia=" + id;
    data += "&fechai=" + feci;
    data += "&fechaf=" + fecf;
    $.post(url, data, function (objJson) {
        ...
        if (lista.length > 0) {
            ...
            for (var i = 0; i < lista.length; i++) {
                ...
                data = {ministerio: lista[i].ministerio, porcentaje: porcen};
                datos.push(data);
            }
            Morris.Bar({
                element: 'graph_bar',
...
function createGM(m) {
    ...
    g += '<div class="x_panel">';
    g += '<div class="x_title">';
    g += '<h2><i class="fa fa-pie-chart"></i>   Miembros Actuales de ' + $("#nIglesiaUser").val() + '</h2>';
    g += '<div class="clearfix"></div>';
    g += '</div>';
    if (m == 1) {
        g += '<div class="x_content">';
...
function graphM() {
    ...
    $.post(url, data, function (objJson) {
        ...
        if (lista.length > 0) {
            ...
            var data = {};
            for (var i = 0; i < lista.length; i++) {
                ministerios[i] = lista[i].ministerio;
                data = {value: lista[i].miembros, name: lista[i].ministerio};
                datos.push(data);
            }
...
function dataFechas(fechai, fechaf) {
    ...
    $("#fechas").empty();
    $("#fechas").append("<i class='fa fa-bar-chart-o'></i>   Reporte de Asistencias a los Ministerios entre " + di + " y " + dif + "");
}